...
def standup_start_v1(token, channel_id, length):
    url = config.url
    return requests.post(f"{url}standup/start/v1", json={'token': token, 'channel_id': channel_id, 'length': length})

def standup_active_v1(token, channel_id):
    url = config.url
    return requests.get(f"{url}standup/active/v1", params={'token': token, 'channel_id': channel_id})

def standup_send_v1(token, channel_id, message):
    url = config.url
    return requests.post(f"{url}standup/send/v1", json={'token': token, 'channel_id': channel_id, 'message': message})
...
def create_valid_user_data():
    ...
    """
    clear_v1()
    user1 = auth_register_v2('validemail@gmail.com', '123abc!@#', 'a', 'b').json()
    user2 = auth_register_v2('secondemail@gmail.com', '123abc!@#', 'c', 'd').json()
    user3 = auth_register_v2('thirdemail@gmail.com', '123abc!@#', 'e', 'f').json()
    user4 = auth_register_v2('fourthemail@gmail.com', '123abc!@#', 'g', 'h').json()
    users = [user1, user2, user3, user4]

    # Create 3 channels with owners user1, user2 & user3 respectively
    channel_1 = channels_create_v2(user1['token'], "General", True).json()
    channel_2 = channels_create_v2(user2['token'], "Music", True).json()
    channel_3 = channels_create_v2(user3['token'], "Study", True).json()
...
def test_standup_start_error(create_valid_user_data):
    ...
    assert InputError == standup_start_v1(users[0]['token'], 999, time_sleep).status_code
    assert InputError == standup_start_v1(users[1]['token'], 111, time_sleep).status_code
    assert InputError == standup_start_v1(users[0]['token'], 543, time_sleep).status_code

    #user not in the channel
    assert AccessError == standup_start_v1(users[0]['token'], channels[1]['channel_id'], time_sleep).status_code
    assert AccessError == standup_start_v1(users[2]['token'], channels[1]['channel_id'], time_sleep).status_code
    assert AccessError == standup_start_v1(users[1]['token'], channels[2]['channel_id'], time_sleep).status_code

    standup_start_v1(users[0]['token'], channels[0]['channel_id'], time_sleep)

    #a standup has been started
    assert InputError == standup_start_v1(users[0]['token'], channels[0]['channel_id'], time_sleep).status_code
    assert InputError == standup_start_v1(users[1]['token'], channels[0]['channel_id'], time_sleep).status_code
    assert InputError == standup_start_v1(users[2]['token'], channels[0]['channel_id'], time_sleep).status_code
    time.sleep(time_sleep)

    standup_start_v1(users[1]['token'], channels[1]['channel_id'], time_sleep)
    #a standup has been started
    
    assert InputError == standup_start_v1(users[1]['token'], channels[1]['channel_id'], time_sleep).status_code
    assert InputError == standup_start_v1(users[3]['token'], channels[1]['channel_id'], time_sleep).status_code
    time.sleep(time_sleep)
...
def test_standup_active_error(create_valid_user_data):
    ...
    #invalid channel id
    assert InputError == standup_active_v1(users[0]['token'], 999).status_code
    assert InputError == standup_active_v1(users[1]['token'], 999).status_code
    assert InputError == standup_active_v1(users[2]['token'], 999).status_code
    assert InputError == standup_active_v1(users[3]['token'], 999).status_code
    clear_v1()
...
def test_standup_send_error(create_valid_user_data):
    ...
    #channel ids are not valids
    assert InputError == standup_send_v1(users[0]['token'], 999, "aaa").status_code
    assert InputError == standup_send_v1(users[1]['token'], 111, "bbb").status_code
    assert InputError == standup_send_v1(users[0]['token'], 543, "ccc").status_code
    ...
    assert InputError == standup_send_v1(users[0]['token'], channels[0]['channel_id'], very_long_message_1).status_code
    assert InputError == standup_send_v1(users[1]['token'], channels[1]['channel_id'], very_long_message_2).status_code
    assert InputError == standup_send_v1(users[2]['token'], channels[2]['channel_id'], very_long_message_3).status_code

    #a standup is not running in the channel
    assert InputError == standup_send_v1(users[0]['token'], channels[0]['channel_id'], "aaa").status_code
    assert InputError == standup_send_v1(users[1]['token'], channels[1]['channel_id'], "aaa").status_code
    assert InputError == standup_send_v1(users[2]['token'], channels[2]['channel_id'], "aaa").status_code
    ...
    #user not in the channel
    assert AccessError == standup_send_v1(users[0]['token'], channels[1]['channel_id'], "aaa").status_code
    assert AccessError == standup_send_v1(users[2]['token'], channels[1]['channel_id'], "aaa").status_code
    assert AccessError == standup_send_v1(users[1]['token'], channels[2]['channel_id'], "aaa").status_code
...
def test_correct_standup_short_message(create_valid_user_data):
    ...
    rt_val = standup_start_v1(users[0]['token'], channels[0]['channel_id'], time_sleep).json()
    ...
    msgs = channel_messages_v2(users[0]['token'], channels[0]['channel_id'], 0).json()
    for i in range(10):
...
def test_correct_standup_active(create_valid_user_data):
    ...
    assert standup_active_v1(users[0]['token'], channels[0]['channel_id']).json()['is_active'] == True
    assert standup_active_v1(users[0]['token'], channels[0]['channel_id']).json()['time_finish'] == time_finish
    
    time.sleep(time_sleep + 1)
    ###################channel 1 standup finishes######################
    assert standup_active_v1(users[0]['token'], channels[0]['channel_id']).json()['is_active'] == False
    assert standup_active_v1(users[0]['token'], channels[0]['channel_id']).json()['time_finish'] == None
    ...
    assert standup_active_v1(users[1]['token'], channels[1]['channel_id']).json()['is_active'] == True
    assert standup_active_v1(users[1]['token'], channels[1]['channel_id']).json()['time_finish'] == time_finish
    
    time.sleep(time_sleep + 1)
    ###################channel 2 standup finishes######################
    assert standup_active_v1(users[1]['token'], channels[1]['channel_id']).json()['is_active'] == False
    assert standup_active_v1(users[1]['token'], channels[1]['channel_id']).json()['time_finish'] == None
...
def test_notification_in_standup(create_valid_user_data):
    ...
    assert notifications_get_v1(users[0]['token']).json() == {
        'notifications': [
            {
    ...
    assert notifications_get_v1(users[1]['token']).json() == {
        'notifications': [
            {
                'channel_id': channels[0]['channel_id'],
                'dm_id': -1,
    ...
    assert notifications_get_v1(users[2]['token']).json() == {
        'notifications': [
            {
                'channel_id': channels[0]['channel_id'],
                'dm_id': -1,
