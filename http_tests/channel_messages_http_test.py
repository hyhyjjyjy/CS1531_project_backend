...
from http_tests.helper_function_for_http_test import auth_register_v2, channel_messages_v2, channels_create_v2, message_send_v2, clear_v1
...
def test_success_less_than_fifty(create_input):
    results = channel_messages_v2(create_input[0][0]["token"], create_input[1][0]["channel_id"], 0).json()
...
def test_success_more_than_fifty(create_input):
    results = channel_messages_v2(create_input[0][1]["token"], create_input[1][1]["channel_id"], 0).json()
    assert type(results) == dict
    ...
    results = channel_messages_v2(create_input[0][1]["token"], create_input[1][1]["channel_id"], 50).json()
    assert type(results) == dict
...
def test_success_more_than_hundred(create_input):
    results = channel_messages_v2(create_input[0][2]["token"], create_input[1][2]["channel_id"], 0).json()
    assert type(results) == dict
    assert len(results["messages"]) == 50
    ...
    results = channel_messages_v2(create_input[0][2]["token"], create_input[1][2]["channel_id"], 50).json()
    assert type(results) == dict
    assert len(results["messages"]) == 50
    assert results["messages"][2]["u_id"] == create_input[0][2]["auth_user_id"]
    ...
    results = channel_messages_v2(create_input[0][2]["token"], create_input[1][2]["channel_id"], 100).json()
    assert type(results) == dict
...
def test_success_start_not_zero_less_fifty(create_input):
    results = channel_messages_v2(create_input[0][0]["token"], create_input[1][0]["channel_id"], 6).json()
    assert type(results) == dict
...
def test_success_start_not_zero_more_fifty(create_input):
    results = channel_messages_v2(create_input[0][1]["token"], create_input[1][1]["channel_id"], 48).json()
    assert type(results) == dict
    ...
    results = channel_messages_v2(create_input[0][2]["token"], create_input[1][2]["channel_id"], 60).json()
    assert type(results) == dict
    assert len(results["messages"]) == 50
    assert results["messages"][0]["u_id"] == create_input[0][2]["auth_user_id"]
    ...
    results = channel_messages_v2(create_input[0][2]["token"], create_input[1][2]["channel_id"], 110).json()
    assert type(results) == dict
    assert len(results["messages"]) == 10
    assert results["messages"][0]["u_id"] == create_input[0][2]["auth_user_id"]
...
def test_start_greater_than_total_messages(create_input):
    assert channel_messages_v2(create_input[0][0]["token"], create_input[1][0]["channel_id"], 23).status_code == INPUT_ERROR
    assert channel_messages_v2(create_input[0][1]["token"], create_input[1][1]["channel_id"], 77).status_code == INPUT_ERROR
    assert channel_messages_v2(create_input[0][2]["token"], create_input[1][2]["channel_id"], 4235).status_code == INPUT_ERROR

def test_not_member(create_input):
    assert channel_messages_v2(create_input[0][0]["token"], create_input[1][1]["channel_id"], 0).status_code == ACCESS_ERROR
    assert channel_messages_v2(create_input[0][1]["token"], create_input[1][2]["channel_id"], 0).status_code == ACCESS_ERROR
    assert channel_messages_v2(create_input[0][2]["token"], create_input[1][0]["channel_id"], 0).status_code == ACCESS_ERROR

def test_channel_id_invalid(create_input):
    # channel_id 5 has not been created
    assert channel_messages_v2(create_input[0][0]["token"], 5, 0).status_code == INPUT_ERROR

def test_token_invalid(create_input):
    # token does not exist
    assert channel_messages_v2("invalidtoken", create_input[1][0]["channel_id"], 0).status_code == ACCESS_ERROR
