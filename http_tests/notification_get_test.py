...
from http_tests.helper_function_for_http_test import dm_create_v1, dm_invite_v1
from http_tests.helper_function_for_http_test import message_send_v2, message_senddm_v1, message_react_v1
from http_tests.helper_function_for_http_test import user_profile_v2, channel_addowner_v1
from http_tests.helper_function_for_http_test import channels_create_v2, channel_join_v2, channel_invite_v2, channel_details_v2
from http_tests.helper_function_for_http_test import auth_register_v2, notifications_get_v1
...
def create_input():
    ...
    data_test_users_handles = [
        user_profile_v2(data_test_users[0]['token'], data_test_users[0]['auth_user_id']).json(),
        user_profile_v2(data_test_users[0]['token'], data_test_users[1]['auth_user_id']).json(),
        user_profile_v2(data_test_users[0]['token'], data_test_users[2]['auth_user_id']).json(),
        user_profile_v2(data_test_users[0]['token'], data_test_users[3]['auth_user_id']).json(),
        user_profile_v2(data_test_users[0]['token'], data_test_users[4]['auth_user_id']).json(),
    ]
    ...
    data_test_channels = [
        ...
        channels_create_v2(data_test_users[1]['token'], 'Channel 2', True).json(),
...
def test_no_notifications():
    ...
    user_4 = auth_register_v2("jordanmilch@mail.com", "password3", "Jordan", "Milch").json()
    user_5 = auth_register_v2("deanzworestine@mail.com", "4thpassword", "Dean", "Zworestine").json()
    
    assert notifications_get_v1(user_1['token']).json() == {'notifications': []}
    assert notifications_get_v1(user_2['token']).json() == {'notifications': []}
    assert notifications_get_v1(user_3['token']).json() == {'notifications': []}
    assert notifications_get_v1(user_4['token']).json() == {'notifications': []}
    assert notifications_get_v1(user_5['token']).json() == {'notifications': []}
...
def test_invalid_token():
    ...
    clear_v1()
    # Token doesnt exist
    user_1 = auth_register_v2('ericzheng@gmail.com', 'happydays1', 'Eric', 'Zheng').json()
    
    assert notifications_get_v1(user_1['token'] + 'bug').status_code == ACCESS_ERROR

    # User not in channel
    user_2 = auth_register_v2('joshhatton@gmail.com', 'happydays2', 'Josh', 'Hatton').json()
    channel_1 = channels_create_v2(user_1['token'], 'Channel 1', True).json()
    
    user_2_handle = user_profile_v2(user_1['token'], user_2['auth_user_id']).json()
    ...
    user_3 = auth_register_v2('bunnydong@gmail.com', 'hihihi!!!', 'Bunny', 'Dong').json()
    user_3_handle = user_profile_v2(user_1['token'], user_3['auth_user_id']).json()
...
def test_greater_than_20_notifications():
    ...
    """
    clear_v1()
    user_1 = auth_register_v2('ericzheng@gmail.com', 'kjhfbshjbdf', 'Eric', 'Zheng').json()
    user_1_handle = user_profile_v2(user_1['token'], user_1['auth_user_id']).json()
...
def test_single_react_notification():
    clear_v1()
    user1 = auth_register_v2("ericzheng@mail.com", "finalpassword", "Eric", "Zheng").json()
    user2 = auth_register_v2("joshhatton@mail.com", "validpassword", "Josh", "Hatton").json()
    channel1 = channels_create_v2(user1['token'], 'Channel 1', True).json()
    channel_join_v2(user2['token'], channel1['channel_id'])   
    msg1 = message_send_v2(user1['token'], channel1['channel_id'], "1 like = 1 prayer").json()
...
def test_multiple_react_notification():
    ...
    user1 = auth_register_v2("ericzheng@mail.com", "finalpassword", "Eric", "Zheng").json()
    user2 = auth_register_v2("joshhatton@mail.com", "validpassword", "Josh", "Hatton").json()
    user3 = auth_register_v2("bunnydong@mail.com", "anotherpassword", "Bunny", "Dong").json()
    user4 = auth_register_v2("jordanmilch@mail.com", "password3", "Jordan", "Milch").json()
    user5 = auth_register_v2("deanzworestine@mail.com", "4thpassword", "Dean", "Zworestine").json()
    channel1 = channels_create_v2(user1['token'], 'Channel 1', True).json()
    channel_join_v2(user2['token'], channel1['channel_id'])
...
def test_assorted_notifications():
    ...
    msg1 = message_send_v2(user1['token'], channel1['channel_id'], msg1str).json()
    message_react_v1(user2['token'], msg1['message_id'], 1)   
    msg2str = "Thanks for having me @ericzheng"
    msg2 = message_send_v2(user2['token'], channel1['channel_id'], msg2str).json()
