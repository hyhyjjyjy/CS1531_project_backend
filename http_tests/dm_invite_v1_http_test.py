...
def dm_invite_v1(token, dm_id, u_id):
    url = config.url
    rt = requests.post(f"{url}dm/invite/v1", json={'token': token, 'dm_id': dm_id, 'u_id': u_id})
    return rt
################################ helper function ##############################################
...
#test if errors occur when we are passing wrong values into the dm_invite function
def test_dm_invite_errors(create_dms):
    ...
    #wrong dm id
    rt = dm_invite_v1(users[0]['token'], 100, users[0]['auth_user_id'])
    assert rt.status_code == InputError

    rt = dm_invite_v1(users[1]['token'], 100, users[0]['auth_user_id'])
    assert rt.status_code == InputError

    #wrong u_id
    rt = dm_invite_v1(users[0]['token'], dm_1['dm_id'], 100)
    assert rt.status_code == InputError

    rt = dm_invite_v1(users[1]['token'], dm_2['dm_id'], 432)
    assert rt.status_code == InputError

    #authetication error
    rt = dm_invite_v1(users[3]['token'], dm_1['dm_id'], users[3]['auth_user_id'])
    assert rt.status_code == AccessError

    #authetication error
    rt = dm_invite_v1(users[3]['token'], dm_2['dm_id'], users[3]['auth_user_id'])
    assert rt.status_code == AccessError

    #invalid token
    rt = dm_invite_v1("fef", dm_2['dm_id'], users[2]['auth_user_id'])
    assert rt.status_code == AccessError
    
    clear_v1()


#test if dm invite works when a proper user is invited into a proper dm
