...
from src.channel import channel_invite_v2, channel_details_v2, channel_addowner_v1
...
def test_invalid_user_id_exception():
    ...
    user = auth_register_v2("ericzheng@mail.com", "peterpiper", "Eric",
                              "Zheng")
    channel = channels_create_v2(user['token'], "Lalaland", True)

    # Test for InputError when invalid users are invited to channel
    with pytest.raises(InputError):
        channel_invite_v2(user['token'], channel['channel_id'],
                          user['auth_user_id'] + 5)
        channel_invite_v2(user['token'], channel['channel_id'],
...
def test_unauthorised_user_exception(create_input):
    ...
    user_in_channel = auth_register_v2("existence@mail.com", "wawaweewa", 
                                       "Already", "Exists")
    channel = channels_create_v2(user_in_channel['token'], "Library", True)
    
    with pytest.raises(AccessError):
        channel_invite_v2(user_1['token'], channel['channel_id'],
                          user_2['auth_user_id'])
        channel_invite_v2(user_2['token'], channel['channel_id'],
                          user_3['auth_user_id'])
        channel_invite_v2(user_3['token'], channel['channel_id'],
                          user_4['auth_user_id'])
        channel_invite_v2(user_4['token'], channel['channel_id'],
                          user_5['auth_user_id'])
        channel_invite_v2(user_5['token'], channel['channel_id'],
                          user_1['auth_user_id'])
...
def test_order_of_exceptions():
    ...
    with pytest.raises(AccessError):
        channel_invite_v2(user_2['token'] + 'bug', channel['channel_id'] + 5,
                            user_test['auth_user_id'] + 5)

    # Pass in invalid user_id, valid token, invalid channel_id,
    # auth_user not part of the channel. This should raise an access error.
    with pytest.raises(InputError):
        channel_invite_v2(user_2['token'], channel['channel_id'] + 5,
                            user_test['auth_user_id'] + 5)
